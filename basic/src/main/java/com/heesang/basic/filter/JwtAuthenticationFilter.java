package com.heesang.basic.filter;
...
import com.heesang.basic.provider.JwtProvider;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // 2. 인증 토큰을 발급받은 후 매 요청마다 인증 토큰을 reques header의 Authorization 필드의 값으로 Bearer 토큰을 포함하여 요청
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // 4. 다음 필터에 requesst 객체와 response 객체를 전달
        filterChain.doFilter(request, response);
    ...
    private String parseBearerToken(HttpServletRequest request) {
        ...
        // Bearer qwkjfsdkohsaskdwdsk // Bearer 7
